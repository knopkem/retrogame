...
namespace GameEngine.Tools {
  ...
  internal struct CompiledVideoElement : ICompiledElement {
    private readonly Color color;
    private readonly VideoPlayer videoPlayer;
    private readonly Vector2 size;
    private readonly Vector2 scale;

    public CompiledVideoElement(Video video, Color color, Vector2 position, Vector2 scale)
      : this() {
      videoPlayer = new VideoPlayer { IsLooped = true };
      Position = position;
      this.color = color;
      this.scale = scale;
      size = new Vector2(video.Width, video.Height);
    ...
    public void Draw(SpriteBatch spriteBatch, Vector2 offset) {
      var origin = new Vector2(0, size.Y / 2f);
      spriteBatch.Draw(videoPlayer.GetTexture(), offset + Position, null, color, 0, origin, scale, SpriteEffects.None, 0);
    }
    #endregion
  }
  ...
  public class MarkupTextEngine {
    public readonly Func<string, bool> ConditionalResolver;
    public readonly Func<string, SpriteFont> FontResolver;
    public readonly Func<string, Texture2D> ImageResolver;
    public readonly Func<string, Video> VideoResolver;

    public MarkupTextEngine(Func<string, SpriteFont> fontResolver, Func<string, Texture2D> imageResolver, Func<string, bool> conditionalResolver, Func<string, Video> videoResolver) {
        ...
        switch (reader.NodeType) {
          case XmlNodeType.Element:
            switch (reader.Name) {
              case "text": {
                  SpriteFont font;
                  string s = reader.GetAttribute("font");
              ...
              case "video": {
                  ...
                  string videoSrc = reader.GetAttribute("src");
                  Color color = Color.White;
                  string s = reader.GetAttribute("color");
                  if (!string.IsNullOrEmpty(s)) {
                    color = ToColor(s);
                  }
                  Video video = VideoResolver.Invoke(videoSrc);
                  if (position.X + video.Width > width) {
                    position = WrapLine(position, lineBuffer, out currentLineHeight, compiledMarkup);
                    currentTotalHeight += currentLineHeight;
                  }
                  Vector2 scale = Vector2.One;
                  ...
                  lineBuffer.Add(new CompiledVideoElement(video, color, position, scale));
                  position.X += video.Width * scale.X;
                  currentLineWidth += video.Width * scale.Y;
                  break;
                }
              case "nbsp": {
